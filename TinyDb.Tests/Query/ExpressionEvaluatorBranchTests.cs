...
public class ExpressionEvaluatorBranchTests
{
    ...
    {
        await Assert.That(QueryValueComparer.Compare(null, null)).IsEqualTo(0);
        await Assert.That(QueryValueComparer.Compare(BsonNull.Value, null)).IsEqualTo(0);
        await Assert.That(QueryValueComparer.Compare(null, 1)).IsEqualTo(-1);
        await Assert.That(QueryValueComparer.Compare(1, null)).IsEqualTo(1);
    }

    [Test]
    public async Task Compare_BsonWrappers_ShouldWork()
    {
        await Assert.That(QueryValueComparer.Compare(new BsonInt32(1), 1)).IsEqualTo(0);
        await Assert.That(QueryValueComparer.Compare(1, new BsonInt32(1))).IsEqualTo(0);
    }
    ...
    public async Task Compare_NumericTypes_ShouldWork()
    {
        ...
        await Assert.That(QueryValueComparer.Compare(d128, 10.5m)).IsEqualTo(0);
        await Assert.That(QueryValueComparer.Compare(10.5m, d128)).IsEqualTo(0);
        await Assert.That(QueryValueComparer.Compare(10, 10.0)).IsEqualTo(0);
        await Assert.That(QueryValueComparer.Compare(10m, 10)).IsEqualTo(0);
    }
    ...
    public async Task Compare_ByteArrays_ShouldWork()
    {
        ...
        await Assert.That(QueryValueComparer.Compare(b1, b2)).IsEqualTo(0);
        await Assert.That(QueryValueComparer.Compare(b1, b3)).IsLessThan(0);
        await Assert.That(QueryValueComparer.Compare(b3, b1)).IsGreaterThan(0);
        await Assert.That(QueryValueComparer.Compare(b1, b4)).IsGreaterThan(0);
    }
    ...
    public async Task Compare_IComparable_ShouldWork()
    {
        ...
        await Assert.That(QueryValueComparer.Compare(o1, o2)).IsLessThan(0);
    }
    ...
    public async Task Compare_ToStringFallback_ShouldWork()
    {
        ...
        await Assert.That(QueryValueComparer.Compare(o1, o2)).IsEqualTo(0);
    }
    ...
    public async Task EvaluateMathOp_Integers_ShouldReturnIntOrLong()
    {
        ...
        var intRes = ExpressionEvaluator.EvaluateMathOp(1, 2, addD, addM);
        await Assert.That(intRes).IsTypeOf<int>();

        long l1 = 1;
        long l2 = 2;
        var longRes = ExpressionEvaluator.EvaluateMathOp(l1, l2, addD, addM);
        ...
        var bigRes = ExpressionEvaluator.EvaluateMathOp(int.MaxValue, 1, addD, addM);
