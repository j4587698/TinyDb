...
public class ComplexQueryTests : IDisposable
{
    ...
    {
        var collection = _engine.GetCollection<Product>();
        collection.Insert(new Product { Id = 1, Name = "Apple iPhone", Category = "Electronics" });
        collection.Insert(new Product { Id = 2, Name = "Samsung Galaxy", Category = "Electronics" });
        collection.Insert(new Product { Id = 3, Name = "Banana", Category = "Food" });

        // StartsWith
        var startsWith = _executor.Execute<Product>("Products", p => p.Name.StartsWith("Apple")).ToList();
        await Assert.That(startsWith.Count).IsEqualTo(1);
        await Assert.That(startsWith[0].Name).IsEqualTo("Apple iPhone");

        // EndsWith
        var endsWith = _executor.Execute<Product>("Products", p => p.Name.EndsWith("Galaxy")).ToList();
        await Assert.That(endsWith.Count).IsEqualTo(1);
        await Assert.That(endsWith[0].Name).IsEqualTo("Samsung Galaxy");

        // Contains
        var contains = _executor.Execute<Product>("Products", p => p.Category.Contains("Electro")).ToList();
    ...
    {
        var collection = _engine.GetCollection<Product>();
        collection.Insert(new Product { Id = 1, Price = 100 });
        collection.Insert(new Product { Id = 2, Price = 200 });
        collection.Insert(new Product { Id = 3, Price = 300 });

        var results = _executor.Execute<Product>("Products", p => p.Price < 150 || p.Price > 250).ToList();
    ...
    public async Task Query_With_Index_Should_Use_Index()
    {
        ...
        collection.Insert(new Product { Id = 1, Name = "A", Category = "Electronics" });
        collection.Insert(new Product { Id = 2, Name = "B", Category = "Food" });
        collection.Insert(new Product { Id = 3, Name = "C", Category = "Electronics" });

        // This query should use the index on Category
        var results = _executor.Execute<Product>("Products", p => p.Category == "Electronics").ToList();
    ...
    {
        var collection = _engine.GetCollection<Product>();
        collection.Insert(new Product { Id = 1, Name = "A", Category = "Electronics", Price = 1000 });
        collection.Insert(new Product { Id = 2, Name = "B", Category = "Electronics", Price = 500 });
        collection.Insert(new Product { Id = 3, Name = "C", Category = "Food", Price = 10 });

        // (Category == "Electronics" && Price < 800) || Category == "Food"
        // Should match Product 2 and 3
        var results = _executor.Execute<Product>("Products", 
    ...
    public async Task Query_With_Unsupported_Expression_Should_Throw()
    {
        ...
        // Using a method that is likely not supported (e.g., ToLower())
        await Assert.That(() => _executor.Execute<Product>("Products", p => p.Name.ToLower() == "apple").ToList())
