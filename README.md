...
### 性能基准测试
| 操作类型 | 数据量 | 平均耗时 | 每秒吞吐量 (OPS) | 备注 |
|---|---|---|---|---|
| **单条插入** | 1000 | 12.38 ms/条 | ~80 | 同步日志模式下受限于IO |
| **批量插入** | 1000 | **8.39 ms/条** | **~119** | 相比单条插入提升 ~32% |
| **主键查询** | 1000 | < 1 ms | > 1000 | B+树索引查找 |
| **无索引查询** | 1000 | ~445 ms | ~2.2 | 全表扫描 + 反序列化 |

### 核心优化
1.  **查询优化**: 实现了 Predicate Pushdown（谓词下推），在 BSON 文档反序列化之前直接评估查询条件，显著减少了全表扫描时的内存分配和 CPU 开销。
2.  **执行计划**: 引入了 `QueryOptimizer`，能够自动选择最优索引或执行策略（IndexSeek vs IndexScan vs FullScan）。
3.  **AOT 兼容**: 核心引擎完全支持 Native AOT 编译，适用于无反射的受限环境。
